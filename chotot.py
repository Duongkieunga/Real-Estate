...
def Crawler():
    set_id = set()
    with open("./crawl/chotot.txt","r") as f:
    ...
    for i in range(6600):
        ...
        page = "https://gateway.chotot.com/v1/public/ad-listing?region_v2=12000&cg=1000&limit=20&o="+str(i*20)+"&st=w&page=1"+str(i+1)
        try:
        ...
        for j in range(len(data['ads'])):
            ...
            try:
                with urllib.request.urlopen("https://gateway.chotot.com/v1/public/ad-listing/"+str(data['ads'][j]['list_id'])) as url1:
                    ...
                    n_Page+=1
                    json.dump(data1,open("./Datas/chotot/"+str(n_Page)+".txt","w",encoding="utf8"), ensure_ascii=False)
                    set_id.add(str(data['ads'][j]['list_id']))
                    with open("./crawl/chotot.txt","a") as f_id:
